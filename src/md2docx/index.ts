...
import { AlignmentType, HeadingLevel, WidthType } from "docx";
import {
  type FileChild,
  type ParagraphChild,
  type BookmarkNode,
...
const handlers: Handlers = {
  ...
  * image(c, ctx): Iterable<FileChild> {
    ...
    const size = {
      width: normalizeAsEMU(`${c.node.data?.meta?.width ?? 10}pt`) / 9525,
      height: normalizeAsEMU(`${c.node.data?.meta?.height ?? 10}pt`) / 9525
    };
